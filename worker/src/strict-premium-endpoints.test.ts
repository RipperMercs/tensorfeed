...
describe('isStrictPremiumPath', () => {
  ...
  describe('list integrity', () => {
    it('exposes all 60 exact paths', () => {
      ...
      // (route-verdict). funding/exposure + packages/pypi/momentum were
      // already strict.
      expect(STRICT_PREMIUM_PATHS).toHaveLength(60);
      expect(new Set(STRICT_PREMIUM_PATHS).size).toBe(60); // no duplicates
    });
