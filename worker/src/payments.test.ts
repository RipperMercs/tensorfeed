...
import {
  ...
  createQuote,
  mintAdminCredits,
  ADMIN_MINT_CREDITS_CAP,
  ADMIN_MINT_LABEL_MAX_LEN,
} from './payments';
...
describe('mintAdminCredits', () => {
  it('mints a fresh tf_live_ token with the requested balance and writes audit entry', async () => {
    const env = makeEnv();
    ...
    if (!result.ok) return;
    expect(result.token.startsWith('tf_live_')).toBe(true);
    expect(result.credits).toBe(50);
    expect(result.label).toBe('phase-2-audit');
    expect(result.mint_id.startsWith('mnt_')).toBe(true);

    // Token credits record exists with the correct balance
    const credKey = `pay:credits:${result.token}`;
    const credRec = await (env.TENSORFEED_CACHE as unknown as MockKV).get(credKey) as { balance: number; total_purchased: number; agent_ua: string };
    expect(credRec).not.toBeNull();
    expect(credRec.balance).toBe(50);
    expect(credRec.total_purchased).toBe(50);
    expect(credRec.agent_ua).toBe('pwsh-irm');

    // Audit record exists at admin:mint:<id> with the same fields
    const auditRec = await (env.TENSORFEED_CACHE as unknown as MockKV).get(`admin:mint:${result.mint_id}`) as { mint_id: string; credits: number; label: string; client_ip: string; token_short: string };
    expect(auditRec).not.toBeNull();
    expect(auditRec.mint_id).toBe(result.mint_id);
    expect(auditRec.credits).toBe(50);
    expect(auditRec.label).toBe('phase-2-audit');
    expect(auditRec.client_ip).toBe('203.0.113.42');
    expect(auditRec.token_short).toBe(result.token.slice(0, 16));
  });

  it('rejects credits over the per-call cap', async () => {
    const env = makeEnv();
    const result = await mintAdminCredits(env, {
      credits: ADMIN_MINT_CREDITS_CAP + 1,
      agentUa: 'test',
    });
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.error).toBe('credits_cap_exceeded');
  });

  it('accepts credits exactly at the cap', async () => {
    const env = makeEnv();
    const result = await mintAdminCredits(env, {
      credits: ADMIN_MINT_CREDITS_CAP,
      agentUa: 'test',
    });
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.credits).toBe(ADMIN_MINT_CREDITS_CAP);
  });

  it('rejects non-positive or non-integer credits', async () => {
    const env = makeEnv();
    expect((await mintAdminCredits(env, { credits: 0, agentUa: 'test' })).ok).toBe(false);
    expect((await mintAdminCredits(env, { credits: -10, agentUa: 'test' })).ok).toBe(false);
    expect((await mintAdminCredits(env, { credits: 1.5, agentUa: 'test' })).ok).toBe(false);
    expect((await mintAdminCredits(env, { credits: NaN, agentUa: 'test' })).ok).toBe(false);
  });

  it('rejects label longer than the max', async () => {
    const env = makeEnv();
    const result = await mintAdminCredits(env, {
      credits: 10,
      label: 'x'.repeat(ADMIN_MINT_LABEL_MAX_LEN + 1),
      agentUa: 'test',
    });
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.error).toBe('label_too_long');
  });

  it('treats empty label string as null', async () => {
    const env = makeEnv();
    const result = await mintAdminCredits(env, {
      credits: 10,
      label: '   ',
  ...
  it('two separate mints produce different tokens and audit ids', async () => {
    const env = makeEnv();
    const a = await mintAdminCredits(env, { credits: 10, agentUa: 'test' });
    const b = await mintAdminCredits(env, { credits: 10, agentUa: 'test' });
    expect(a.ok && b.ok).toBe(true);
    if (!a.ok || !b.ok) return;
    expect(a.token).not.toBe(b.token);
    expect(a.mint_id).not.toBe(b.mint_id);
  });
});
