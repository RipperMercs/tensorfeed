...
// === Admin-mint: out-of-band credit issuance for testing/ops ===
//
// Mints a fresh tf_live_ bearer token with a specified credit balance,
// authenticated only by ADMIN_KEY (gated at the route layer). No USDC
// payment, no quote, no welcome bonus. Every mint writes a permanent
// audit-log entry to admin:mint:<id> so the off-chain provenance of any
// admin-minted token is forever traceable. Per-call cap is enforced
// here (defense in depth: even if the route forgets to validate, a
// malformed call still cannot exceed the cap).
//
// Use cases:
//  - internal smoke tests of the premium surface (Phase 2 audits)
//  - support cases (re-issue credits after a verified ops failure)
//  - vetted partner trials
// Not for: anything externally addressable. Treat ADMIN_KEY as the
// only gate and rotate it on any compromise suspicion.

export const ADMIN_MINT_CREDITS_CAP = 1000;
export const ADMIN_MINT_LABEL_MAX_LEN = 64;

export type AdminMintResult =
  | {
      ok: true;
      token: string;
      credits: number;
...
export interface AdminMintAuditEntry {
  mint_id: string;
  token_short: string;
  credits: number;
  label: string | null;
...
export async function mintAdminCredits(
  env: Env,
  args: { credits: number; label?: string | null; agentUa: string; clientIp?: string | null },
): Promise<AdminMintResult> {
  if (!Number.isInteger(args.credits) || args.credits < 1) {
    return { ok: false, error: 'invalid_credits', reason: 'credits must be a positive integer' };
  }
  if (args.credits > ADMIN_MINT_CREDITS_CAP) {
    return {
      ok: false,
      error: 'credits_cap_exceeded',
      reason: `Per-call cap is ${ADMIN_MINT_CREDITS_CAP}. Issue multiple smaller mints if more is needed; each writes its own audit entry.`,
    };
  }
  let label: string | null = null;
  if (args.label !== undefined && args.label !== null) {
    if (typeof args.label !== 'string') {
      return { ok: false, error: 'invalid_label', reason: 'label must be a string' };
    }
    const trimmed = args.label.trim();
    if (trimmed.length > ADMIN_MINT_LABEL_MAX_LEN) {
      return {
        ok: false,
        error: 'label_too_long',
        reason: `label exceeds ${ADMIN_MINT_LABEL_MAX_LEN} chars`,
      };
    }
  ...
  const token = generateToken();
  const mintIdBytes = new Uint8Array(8);
  crypto.getRandomValues(mintIdBytes);
  const mint_id = `mnt_${toHex(mintIdBytes)}`;
  const now = new Date().toISOString();
  ...
  const tokenRecord: CreditsRecord = {
    ...
    created: now,
    last_used: now,
    agent_ua: args.agentUa,
    total_purchased: args.credits,
  };

  const auditEntry: AdminMintAuditEntry = {
    mint_id,
    token_short: token.slice(0, 16),
  ...
  await Promise.all([
    env.TENSORFEED_CACHE.put(`pay:credits:${token}`, JSON.stringify(tokenRecord)),
    env.TENSORFEED_CACHE.put(`admin:mint:${mint_id}`, JSON.stringify(auditEntry)),
  ]);

  return { ok: true, token, credits: args.credits, mint_id, minted_at: now, label };
}
