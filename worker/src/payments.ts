...
export async function logPremiumUsage(
  ...
  try {
    // Commit-outcome bridge (main). Must run before anything is recorded:
    // a zero actual debit means the no-charge rollup already logged the
    // event, so the paid rollup and the event trail both have to skip it.
    if (payment) {
