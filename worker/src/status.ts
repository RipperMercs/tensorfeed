...
export async function pollStatusPages(env: Env): Promise<void> {
  ...
  if (watchTransitions.length > 0) {
    ...
    // A real status transition just happened: push the status pages (hub,
    // homepage, and each flipped provider's is-X-down question page) into
    ...
    await pingIndexNow(
      env,
      statusFlipUrlList(watchTransitions.map((t) => t.provider)),
      `status-flip:${watchTransitions.map((t) => `${t.provider}:${t.from}>${t.to}`).join(',')}`,
    );
  }
}
