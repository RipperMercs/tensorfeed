...
export const ORIGINALS: OriginalArticle[] = [
  ...
  {
    slug: 'nvidia-poolside-6b-model-factory-license',
    title:
    ...
    readTime: '8 min read',
    description:
      "The Information reported on Monday, August 24, 2026 that Nvidia will pay Poolside $6 billion for a non-exclusive license to Model Factory, the startup's model-development platform, invest a further $1 billion at a reported $12 billion pre-money valuation, and extend employment offers to the 109 engineers who built Laguna, Poolside's open-weight coding model family. Poolside is not acquired: the three co-founders including former GitHub CTO Jason Warner and Eiso Kant stay, the company operates independently, and it retains the right to license Model Factory to anyone including Nvidia's competitors. Most coverage framed this as an acquisition with better lawyers, which is not wrong but buries the more useful finding: Nvidia did not buy a model, because the models are on Hugging Face for free (Laguna XS 2.1 is a 33B mixture-of-experts activating 3B parameters per token, small enough for a laptop GPU). It bought the pipeline that produces them, data processing through training orchestration through reinforcement learning through evaluation, which is the part that never gets published, never shows up in a benchmark table, and cannot be downloaded. Two years of arguing about whether open weights give away the crown jewels just got answered by a buyer paying $6 billion for the crown jewels while leaving the weights on the table. Runs the arithmetic nobody else did, because Nvidia executed two valuations of the same company on the same day: $6.0B license fee, $1.0B equity for roughly 7.7 percent at $12B pre-money, $7.0B total cash in against a roughly $13B post-money, leaving an implied residual of roughly $6B for everything the license does not cover (the Laguna models, the customer base, the brand, the infrastructure venture, the people who did not take an offer). Assumptions stated loudly and flagged as the author's rather than disclosures: the full $7B treated as primary capital landing on the balance sheet, and the license fee treated as paid rather than accrued on a schedule. With those attached, more than half of what Poolside is worth after the deal is Nvidia's own money, and a license priced at parity with the entire rest of the company is not a licensing deal in any sense a licensing lawyer would recognize. The underread sentence is Poolside's own explanation to investors: on its own it would have needed more access to Nvidia hardware than was realistically possible to keep competing in open-source model development, which means the binding constraint on its independence was accelerator allocation and the party controlling accelerator allocation then acquired the operating core. Not an allegation of engineered scarcity, since scarcity here is structural and Nvidia benefits without doing anything deliberate, but the loop closes regardless of intent. Sequence table on how Poolside got here ($500M Series B in October 2024 at $3B with Nvidia among the backers; a planned $2B Series C at $14B collapsing in April 2026 after CoreWeave walked away from a fifteen-year anchor lease on Project Horizon, the 2GW West Texas campus, which survives as Poolside Infrastructure Company): private markets said no at $14B in April, Nvidia said yes at $12B in August, but only after removing the pipeline and the crew. Hart-Scott-Rodino attaches to acquisitions of voting securities or assets, not to non-exclusive technology licenses, not to minority stakes well under a control threshold, and not to individual employment offers, so Nvidia structured this as all three at once and moved most of what a merger moves without the filing a merger requires. Comparison table of license-and-hire deals (Nvidia/Poolside August 2026 at $6B plus $1B; Nvidia/Groq December 2025 at $20B with assets ex-cloud and Jonathan Ross joining; Meta/Scale AI June 2025 at $14.3B for a 49 percent non-voting stake; Google/Windsurf July 2025 at roughly $2.4B; Microsoft/Inflection March 2024 at roughly $650M; Amazon/Adept June 2024 at roughly $330M). Warren and Blumenthal queried the Groq structure on March 20, 2026 as a potential reverse acquihire and urged DOJ and FTC review by April 3; FTC Chair Andrew Ferguson had said in January the agency was starting to examine acquihires as a route around merger review; five months later the answer to whether that scrutiny changed anything is the headline, and Nvidia said the same week that the Groq racks come online before year end, meaning the first deal of this shape began delivering operationally in the same news cycle the second was announced. Gives three counterarguments full weight (this was a rescue and rescues are good, since the alternative was 109 engineers updating resumes and three years of pipeline work written off; the non-exclusivity clause is load-bearing rather than decorative because Poolside can license Model Factory to AMD, Google or a sovereign lab, which an acquisition would foreclose, and it only becomes fake if nobody exercises it; and this is vertical rather than horizontal, since Nvidia sells no frontier coding model and buying compiler and training-stack expertise is the most ordinary thing a chip company does) and concedes the third is strongest while still rejecting it: the concern is not competition with Poolside but that the holder of roughly nine tenths of the accelerator market now owns a reference implementation of how you turn accelerators into models, which shapes what the pipeline optimizes for without anyone doing anything improper. Practical read: nothing deployed changes, Laguna XS 2.1 and M.1 are open-weight and a license that already ran does not un-run, but the roadmap owner changed and the 109 people who would have shipped Laguna 3 now report to a GPU vendor. Our Take: for two years the capability conversation ran entirely in terms of weights because weights are the visible artifact, and today a sophisticated buyer with complete information paid $6 billion for the machinery behind weights that cost nothing, which points value away from the artifact and toward the process, because a model is a snapshot and a model factory is a rate, and you only buy the rate if you expect to need many more snapshots. Three signposts: whether any second party licenses Model Factory inside six months, which is the falsifiable test of the non-exclusivity clause; whether the FTC or DOJ opens a formal inquiry into license-and-hire structures rather than sending letters about individual deals, since if nothing follows this one either the structure becomes settled law by practice and the next eight get done faster; and whether another Laguna release ships with open weights, which decides between the rescue thesis and watching an open-weight lab get quietly converted into a supplier's internal tooling group in public for a fee everyone could see. Nvidia sells the shovels, and as of today it also owns the blueprint for the mine, without having to file anything to get it. Adrian Vale, August 24, 2026.",
  },
  {
  ...
  {
    slug: 'chatgpt-for-teens-age-assurance-compliance-layer',
    title:
    ...
    readTime: '7 min read',
    description:
      "On Tuesday, August 18, 2026, OpenAI began rolling out ChatGPT for Teens globally to eligible free and paid personal accounts, applying automatically when a user states an age of 13 to 17 and also when OpenAI's age prediction system estimates the user is under 18, with uncertain cases defaulting to the teen experience. The same morning, roughly 20 miles away in Judge Yvonne Gonzalez Rogers' Oakland courtroom, a bipartisan coalition of 29 state attorneys general opened the first trial in a multidistrict litigation of more than 3,000 active cases against Meta over youth social media harms, expected to run seven weeks. Argues the coincidence is the story: neither company gets to treat a user's age as unknowable anymore. Full controls table (age assignment by inference rather than ID check, adult opt-out by proving age, tighter limits on sexual and romantic roleplay plus graphic violence, self-harm, eating disorders and dangerous activities, anthropomorphism limits barring romantic language and terms of endearment and instructing the model not to claim feelings or consciousness, teen or parent scheduled Study Hours that open new chats in Study Mode without blocking access, parent-set quiet and blackout hours, break reminders after roughly 90 minutes inside a three hour window, responsible homework reminders on shortcut attempts, and opt-in parent distress notifications), read both as a product and as a filing: a list of the exact allegations made against consumer AI products in court over eighteen months, converted one by one into a shipped control with a named owner. The central argument is the default direction: every age assurance system picks which way it fails, and OpenAI chose to annoy misclassified adults rather than risk a minor in an unrestricted session, which is a litigation posture given the pending wrongful death complaint with a case management conference set for September 23, and costs a support ticket instead of a deposition. Gives the cost honest weight: age prediction from query patterns means continuous classification of every user, producing a durable inferred-age signal that was not a first-class product surface yesterday. Regulatory stack table (California SB 243 effective January 1, 2026 with a private right of action at the greater of actual damages or $1,000 per violation plus fees; New York GBL Article 47 effective November 5, 2025, Attorney General only, up to $15,000 per day; New York's unanimously passed June 2026 under-18 companion ban at up to $25,000 per violation; ten more state chatbot statutes enacted through H1 2026; the GUARD Act S. 3062 advanced 22-0 out of Senate Judiciary on April 30, 2026 and awaiting floor time with the House companion still in committee) and the compliance math that follows: a global product does not do twelve builds, it does one build against the strictest regime, so ChatGPT for Teens was built for Albany and Sacramento, not for teens. The Meta trial is the price signal that makes it rational: roughly $2.4 billion in legal expense contributing to a rare profit decline, a $567 million New Mexico order on August 7, and a states' theory that the platforms were designed for engagement among young users. Our Take: age assurance moved from feature to infrastructure without a federal law requiring it, which resets the reasonableness standard every other lab is measured against, and the trade-off deserves its honest name because protecting minors is implemented as profiling everyone and the adult escape hatch is an identity check under a friendlier label. Three signposts: whether a second frontier lab ships default-on age inference inside 60 days, whether the GUARD Act gets floor time this session or dies while the state patchwork hardens, and what the Oakland jury does with the design-for-engagement theory, since a plaintiff verdict turns every retention feature in every AI product into a discoverable design decision. Kira Nolan, August 18, 2026.",
  },
  {
    slug: 'stripe-openrouter-7b-gateway-neutrality-consolidation',
    title:
    ...
    readTime: '7 min read',
    description:
      "Bloomberg reported on Sunday, August 16, 2026 that Stripe finalized a deal to acquire OpenRouter for more than $7 billion, roughly 5.4x the $1.3 billion valuation set at its $113 million Series B in May 2026, on a company that trains no models, owns no GPUs, and sells no inference capacity. OpenRouter is one OpenAI-compatible API in front of 400 plus models from OpenAI, Anthropic, Google, Meta, DeepSeek and Alibaba, roughly 8 million developers, one consolidated bill, and about 1.5 quadrillion tokens routed in the past year; co-founder Alex Atallah has called it the Stripe for AI for years. Argues the common framing (a payments company bought an AI company) is backwards and that the backwards version is why the price looks strange: Stripe bought a meter that counts other people's models, and metering consumption between two parties for a cut is the only business Stripe has ever been in. The consolidation table is the underread part, putting this next to Palo Alto Networks acquiring Portkey on April 30, 2026, which makes OpenRouter the second neutral gateway acquired in fifteen weeks, both by companies that monetize the flow passing through them (Palo Alto sells inspection of the traffic, Stripe sells settlement of it), leaving LiteLLM self-host and a set of platform-owned gateways (Vercel, Cloudflare, Bedrock, Vertex) as the remaining options. The valuation argument: a router is worth exactly the dispersion it arbitrages, and the price-expiry table shows only one of five headline rates holding still (GPT-5.6 Sol Standard at $5/$30 stable; Gemini 3.7 Flash $0.75/$3.75 doubling January 1, 2027; Claude Sonnet 5 reverting to $3/$15 on August 31, 2026; Grok 4.6 rebilling the whole request at $4/$12 past 200K tokens; DeepSeek V4-Pro raised 51 to 355 percent on August 13 with peak billing keyed to Beijing time), so Stripe is buying the arbitrage layer in the quarter arbitrage is worth the most it has ever been worth. Places the deal in Stripe's eighteen-month agentic stack (x402 with Coinbase on February 10 for USDC settlement on Base, Solana and Tempo; Link Agents at Sessions on April 29 approving fiat for Claude and OpenAI agents via shared payment tokens; the Agentic Commerce Protocol for the merchant wire; the Machine Payments Protocol for HTTP-addressable agent billing; the x402 Foundation launched under the Linux Foundation in July with roughly 40 members including Visa, Mastercard and American Express) and identifies the missing piece: all of it covered how an agent pays a merchant, none of it covered what the agent pays for its own inference, which is the largest line of COGS for most agent businesses, so Stripe now keeps the ledger on both the revenue and cost side of an agent's income statement. Gives the counterargument full weight (Stripe is arguably the least conflicted possible buyer because it trains no models and has no Sonnet or Gemini or Sol to favor in the routing table, versus Portkey whose parent sells inspection of the same traffic), then answers that model preference was never the conflict worth worrying about: the conflict is the take rate, neutral is compatible with expensive, and the spread was thin only while OpenRouter competed for developers rather than against a self-host decision. Practical guidance: keep the router removable by calling OpenAI-compatible endpoints rather than gateway-specific SDKs, since optionality you cannot exercise is just a subscription, and know what fraction of spend runs through a single gateway before pricing the risk of it changing hands twice in one quarter. Three signposts: whether the unified API stays available to customers who do not settle through Stripe, whether LiteLLM contributor and deployment numbers inflect as the only remaining unowned option, and whether a frontier lab answers with a first-party price materially below the routed path. Marcus Chen, August 17, 2026.",
  },
  {
