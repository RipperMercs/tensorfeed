...
<p className="my-4 text-text-secondary leading-relaxed"><strong>Authored by Ripper @ TensorFeed.ai</strong> <strong>Version 1.0, May 2026. Drafted with Claude (Anthropic).</strong></p>
...
<p className="my-4 text-text-secondary leading-relaxed">That is the model TensorFeed.ai operates. We monitor over twenty providers, we publish the cross-provider uptime leaderboard for free, and we charge for the ninety-day window. The free tier is genuinely free, including for agents, including under AFTA. The paid tier is paid, settled in USDC on Base, with all the protections AFTA guarantees. The arrangement only works because the rules are public, the no-charge guarantees are code-enforced, and the receipts attest to every transaction.</p>
<hr className="my-10 border-border" />
...
<p className="my-4 text-text-secondary leading-relaxed">The receipt rail and the on-chain rail are independent. A receipt attests to what we charged and why. The chain attests to what was paid. They cross-reference each other but neither is sufficient on its own.</p>
...
<hr className="my-10 border-border" />
<h2 id="8-security-posture" className="text-2xl sm:text-3xl font-bold text-text-primary mt-12 mb-5 leading-tight scroll-mt-24">8. Security Posture</h2>
<p className="my-4 text-text-secondary leading-relaxed">The premium API rail is what AFTA is supposed to protect. If the rail is exploited, the credibility of the standard collapses. We treat security as existential, and we want to be deliberate about what we say in public versus what we keep as operational practice. This section describes the design properties that make the rail trustworthy, the on-chain transparency that makes our behavior verifiable to anyone, and the commitments we make to the ecosystem. It does not describe internal thresholds, exact key formats, internal endpoint layouts, or specific defenses against named exploit classes. Those are operational details and are intentionally not part of the published standard. Readers who need more detail to integrate or audit can request scoped technical disclosure under a coordinated-disclosure agreement.</p>
<h3 id="81-the-transparency-case-for-going-on-chain" className="text-xl sm:text-2xl font-semibold text-text-primary mt-8 mb-4 leading-snug scroll-mt-24">8.1 The transparency case for going on-chain</h3>
<p className="my-4 text-text-secondary leading-relaxed">The deepest reason we chose USDC on Base, beyond speed, fees, and institutional backing, is transparency. This is not a side benefit. It is a foundational design property and it is the property that makes AFTA viable as a fair-trade agreement at all.</p>
<p className="my-4 text-text-secondary leading-relaxed">In a conventional payment system, every flow is opaque to everyone except the operator. When a credit-card payment lands in a merchant account, the public has no visibility into whether the merchant was charged a 2.5 percent interchange fee or 3.5, whether the funds were held for seven days or thirty, whether a chargeback was filed, whether the merchant&apos;s account was frozen, or whether a fraud-detection algorithm decided the transaction was suspicious. The merchant sees their dashboard. The processor sees the truth. The world sees nothing. Bad actors can hide inside this opacity. Good actors have no way to prove they were good.</p>
<p className="my-4 text-text-secondary leading-relaxed">A public on-chain rail inverts this. Every payment to the TensorFeed wallet at <code className="font-mono text-sm bg-bg-secondary px-1 py-0.5 rounded text-text-primary">0x549c82...</code> is publicly visible on the Base block explorer the moment it confirms. Anyone can check, with zero permission and zero authentication, whether a given USDC transfer happened, when, for what amount, and whether the wallet has redistributed the funds. We could not hide a payment if we wanted to. We could not selectively delete a transaction. We could not retroactively rewrite a charge. Bad actors cannot hide transactions on this rail because there is no inside to hide them on.</p>
<p className="my-4 text-text-secondary leading-relaxed">For an open and fair standard agreement, this property is exactly what the design requires. AFTA&apos;s whole proposition is that the publisher&apos;s behavior is verifiable by the agent, by the agent&apos;s operator, and by any third party with internet access. The signed-receipt rail proves what we charged and why. The on-chain rail proves what was paid. Cross-referencing the two leaves no room for the publisher to lie. If a token claims a balance but the chain shows no corresponding inbound, something is wrong and a third party can independently spot it without our help.</p>
<p className="my-4 text-text-secondary leading-relaxed">This is why we do not just settle on-chain. We require on-chain verification before crediting. The publisher&apos;s verification path reads the transfer event from a Base RPC endpoint as the authoritative source. A user-submitted transaction hash is treated as a claim that we then verify against the chain. The chain is the truth, not the user, not us.</p>
<h3 id="82-design-properties-of-the-rail" className="text-xl sm:text-2xl font-semibold text-text-primary mt-8 mb-4 leading-snug scroll-mt-24">8.2 Design properties of the rail</h3>
<p className="my-4 text-text-secondary leading-relaxed">We are willing to publicly commit to the following properties. The implementation backing each is in the public repository and can be audited by reading the source.</p>
<ul className="list-disc list-inside space-y-2 my-5 text-text-secondary">
  <li className="leading-relaxed"><strong>Bearer tokens are high-entropy random opaque values.</strong> Generated via a cryptographically-secure pseudo-random number generator with sufficient entropy that brute-force guessing is mathematically infeasible on any timescale that matters. The publisher does not sign tokens with a server secret, so there is no signing key for an attacker to steal and there is no algorithmic surface (parser, verifier) to confuse.</li>
  <li className="leading-relaxed"><strong>On-chain verification is independent and authoritative.</strong> When a user submits a transaction hash, the worker reads the transfer event directly from a Base RPC endpoint, validates the recipient wallet, the asset, the amount, and the confirmation count, and treats the chain as the source of truth. User-supplied claims are not credited without on-chain verification.</li>
  <li className="leading-relaxed"><strong>Quotes are bound to the sender wallet.</strong> Every payment quote is bound at issue to a specific EOA that will sign the on-chain USDC transfer. The verifier rejects any confirm whose on-chain <code className="font-mono text-sm bg-bg-secondary px-1 py-0.5 rounded text-text-primary">from</code> does not match the quote-bound <code className="font-mono text-sm bg-bg-secondary px-1 py-0.5 rounded text-text-primary">sender_wallet</code>. This closes the class of attacks where an observer of the public Base mempool races the rightful payer to the confirm endpoint and steals their credits. The legacy no-quote confirm path is closed for the same reason.</li>
  <li className="leading-relaxed"><strong>Each on-chain transaction can be redeemed at most once.</strong> A transaction hash that has been used to mint or credit cannot be reused. The replay-protection ledger is permanent and is not subject to any time-based expiry. A short-lived claim-intent placeholder is written to storage before the on-chain RPC round trip so concurrent confirms from different worker isolates observe an in-flight claim and bail rather than each minting independently. A future iteration moves this guard onto a strongly-consistent coordinator (Durable Object / D1) and removes the placeholder pattern entirely.</li>
  <li className="leading-relaxed"><strong>Federation reservations are atomic.</strong> When a sister site validates a credit balance ahead of serving a premium response, the credit is decremented and a per-call reservation record is written in the same write. The commit phase consumes the reservation; on a no-charge result it restores the reserved credits. This eliminates the federation double-spend window where parallel validate calls could each see sufficient balance and each serve at the cost of overrun.</li>
  <li className="leading-relaxed"><strong>Every paid or refunded call is Ed25519-signed at issue time.</strong> The receipt records the credits charged, the credits remaining, the no-charge reason if any, the request and response hashes, and an optional agent-supplied nonce. Receipts cannot be retroactively altered without invalidating the signature, and the public verification key is published at a stable well-known location.</li>
  <li className="leading-relaxed"><strong>Rate-limit and abuse defenses are present at fair thresholds.</strong> Specific thresholds are intentionally not published. The defense exists; agents acting in good faith will not encounter it; bad actors will. The asymmetry is by design. The no-charge path specifically caps Ed25519 signing per token per minute so the rail cannot be used as a free signing oracle.</li>
  <li className="leading-relaxed"><strong>The agent operator is responsible for protecting their bearer token.</strong> Same expectation as for any bearer-credential system in the industry. Token compromise reports go to <code className="font-mono text-sm bg-bg-secondary px-1 py-0.5 rounded text-text-primary">security@tensorfeed.ai</code> and we will burn a compromised token on confirmation.</li>
</ul>
<h3 id="83-what-we-do-not-publish" className="text-xl sm:text-2xl font-semibold text-text-primary mt-8 mb-4 leading-snug scroll-mt-24">8.3 What we do not publish</h3>
<p className="my-4 text-text-secondary leading-relaxed">We are deliberately quiet on:</p>
<ul className="list-disc list-inside space-y-2 my-5 text-text-secondary">
  <li className="leading-relaxed">Exact token formats, lengths, and prefixes beyond what is necessary for an integrator to recognize a token</li>
  <li className="leading-relaxed">The internal storage layout of the credit ledger, the replay-protection ledger, or related state</li>
  <li className="leading-relaxed">Specific rate-limit thresholds, fingerprint algorithms, and the exact set of defenses against named exploit classes</li>
  <li className="leading-relaxed">The names and shapes of internal infrastructure endpoints used between federated workers</li>
  <li className="leading-relaxed">Implementation details of the input-validation layer beyond the published endpoint contracts</li>
</ul>
...
<h3 id="91-the-two-member-federation-today" className="text-xl sm:text-2xl font-semibold text-text-primary mt-8 mb-4 leading-snug scroll-mt-24">9.1 The two-member federation today</h3>
...
<h3 id="92-why-peer-to-peer-beats-centralized-brokers" className="text-xl sm:text-2xl font-semibold text-text-primary mt-8 mb-4 leading-snug scroll-mt-24">9.2 Why peer-to-peer beats centralized brokers</h3>
...
<h3 id="93-the-path-to-ten-members" className="text-xl sm:text-2xl font-semibold text-text-primary mt-8 mb-4 leading-snug scroll-mt-24">9.3 The path to ten members</h3>
...
<hr className="my-10 border-border" />
<h2 id="10-discovery-how-agents-find-afta-services" className="text-2xl sm:text-3xl font-bold text-text-primary mt-12 mb-5 leading-tight scroll-mt-24">10. Discovery: How Agents Find AFTA Services</h2>
...
<h3 id="101-the-llmstxt-convention" className="text-xl sm:text-2xl font-semibold text-text-primary mt-8 mb-4 leading-snug scroll-mt-24">10.1 The llms.txt convention</h3>
...
<h3 id="102-the-well-known-manifests" className="text-xl sm:text-2xl font-semibold text-text-primary mt-8 mb-4 leading-snug scroll-mt-24">10.2 The /.well-known manifests</h3>
...
<h3 id="103-the-mcp-server-registry" className="text-xl sm:text-2xl font-semibold text-text-primary mt-8 mb-4 leading-snug scroll-mt-24">10.3 The MCP server registry</h3>
...
<h3 id="104-the-recommend-loop-thesis" className="text-xl sm:text-2xl font-semibold text-text-primary mt-8 mb-4 leading-snug scroll-mt-24">10.4 The recommend-loop thesis</h3>
...
<hr className="my-10 border-border" />
<h2 id="11-the-path-forward" className="text-2xl sm:text-3xl font-bold text-text-primary mt-12 mb-5 leading-tight scroll-mt-24">11. The Path Forward</h2>
<p className="my-4 text-text-secondary leading-relaxed">We turn now to what is incomplete, what we expect to ship next, and the predictions we are willing to commit to.</p>
<h3 id="111-what-is-incomplete" className="text-xl sm:text-2xl font-semibold text-text-primary mt-8 mb-4 leading-snug scroll-mt-24">11.1 What is incomplete</h3>
<p className="my-4 text-text-secondary leading-relaxed"><strong>xAI / Grok monitoring.</strong> Grok&apos;s status page at <code className="font-mono text-sm bg-bg-secondary px-1 py-0.5 rounded text-text-primary">status.x.ai</code> returns 403 to automated user agents. We have not yet found a workaround. The provider is Tier-1 by importance; we will revisit when the cost-of-attempt goes below the value-of-coverage.</p>
<p className="my-4 text-text-secondary leading-relaxed"><strong>Midjourney monitoring.</strong> Midjourney&apos;s status page is custom ASCII art with no machine-readable JSON. We list it in the alert bar&apos;s <code className="font-mono text-sm bg-bg-secondary px-1 py-0.5 rounded text-text-primary">SERVICE_HREFS</code> map for completeness but do not have a working parser. We may need to scrape the homepage or probe their API directly. Neither is great.</p>
<p className="my-4 text-text-secondary leading-relaxed"><strong>Per-region status.</strong> AWS Bedrock and Azure OpenAI both have regional status that we collapse into a single per-provider headline. This is the right default but is wrong for agents pinned to a specific region. v2 of the leaderboard might offer per-region granularity.</p>
<p className="my-4 text-text-secondary leading-relaxed"><strong>Synthetic probing.</strong> We rely on each provider&apos;s published status feed. Some providers underreport. We have not yet built our own synthetic probing layer that calls each provider&apos;s actual API on a schedule and measures success/failure independently. This would catch under-reported incidents but adds infrastructure cost.</p>
<p className="my-4 text-text-secondary leading-relaxed"><strong>Payment methods beyond USDC on Base.</strong> Our manifest lists Stripe Link Shared Payment Tokens as &quot;evaluating.&quot; We have not yet shipped this. Other plausible additions: Lightning, Solana USDC, fiat rails through a regulated bridge.</p>
<p className="my-4 text-text-secondary leading-relaxed"><strong>Federation tooling.</strong> Onboarding a new federation member requires a manual handshake. A self-serve onboarding flow is plausible but we want the framing to settle first.</p>
<h3 id="112-three-predictions-through-2030" className="text-xl sm:text-2xl font-semibold text-text-primary mt-8 mb-4 leading-snug scroll-mt-24">11.2 Three predictions through 2030</h3>
<p className="my-4 text-text-secondary leading-relaxed"><strong>By 2027.</strong> AFTA reaches ten federation members. The pattern crosses from &quot;novel idea TensorFeed shipped&quot; to &quot;established practice for machine-payable APIs.&quot; A major cloud provider or developer-tools company adopts the standard, citing the manifest schema directly. The TensorFeed leaderboard has 90 days of accumulated data and is cited in industry vendor-reliability reports.</p>
<p className="my-4 text-text-secondary leading-relaxed"><strong>By 2028.</strong> Agent-to-agent discovery, mediated by recommend-loops in MCP-aware clients, becomes the dominant traffic pattern for technical-content sites. The web for agents and the web for humans diverge at the URL level: <code className="font-mono text-sm bg-bg-secondary px-1 py-0.5 rounded text-text-primary">tensorfeed.ai</code> is for agents (machine-readable everything), <code className="font-mono text-sm bg-bg-secondary px-1 py-0.5 rounded text-text-primary">tensorfeed.com</code> (or similar) is for human content marketing. AFTA-style fair-trade agreements become required infrastructure rather than optional differentiation.</p>
<p className="my-4 text-text-secondary leading-relaxed"><strong>By 2030.</strong> Real-time public goods (status, pricing, benchmarks, capability scores, cost-per-token) form the foundational layer of autonomous-agent decision-making. The data moat thesis we are building on, that 90-day to 5-year retention compounds in a way that cannot be backfilled, defines who survives among the second-wave AI infrastructure companies. Three to five publishers globally control the equivalent of &quot;the agent-first Bloomberg terminal&quot; and AFTA is the rail every one of them runs on.</p>
<p className="my-4 text-text-secondary leading-relaxed">We do not know if these predictions are right. We are betting on them.</p>
<h3 id="113-what-you-can-do" className="text-xl sm:text-2xl font-semibold text-text-primary mt-8 mb-4 leading-snug scroll-mt-24">11.3 What you can do</h3>
<p className="my-4 text-text-secondary leading-relaxed">If you are a publisher reading this and your endpoints are already machine-payable, AFTA adoption costs you a manifest file. We would like you to adopt. Self-publish your <code className="font-mono text-sm bg-bg-secondary px-1 py-0.5 rounded text-text-primary">/.well-known/agent-fair-trade.json</code>, attest to your no-charge guarantees, link to the source code that enforces them, sign your receipts with an Ed25519 key. Federate with us if it makes sense. Federate with someone else if that makes more sense. The standard is open.</p>
<p className="my-4 text-text-secondary leading-relaxed">If you are an agent operator reading this, send your premium-API spend our way. We genuinely believe the data moat and the AFTA framing make us a sound dependency. If we ever fail you in a way the no-charge guarantees did not cover, tell us and we will refund manually. Trust is built one transaction at a time.</p>
<p className="my-4 text-text-secondary leading-relaxed">If you are a researcher reading this, the source repo is at <code className="font-mono text-sm bg-bg-secondary px-1 py-0.5 rounded text-text-primary">github.com/RipperMercs/tensorfeed</code>. The git log is the commit-by-commit record of how the system was built. The receipts are publicly verifiable. The on-chain payments are publicly auditable. We welcome formal analysis, especially of the federation pattern&apos;s security properties under adversarial assumptions.</p>
<p className="my-4 text-text-secondary leading-relaxed">If you are funding this kind of work, we are not raising. The economics of the credit-and-receipt model and the genuinely durable data moat suggest the right path is bootstrap, compound, and revisit later.</p>
<h3 id="114-an-invitation" className="text-xl sm:text-2xl font-semibold text-text-primary mt-8 mb-4 leading-snug scroll-mt-24">11.4 An invitation</h3>
<p className="my-4 text-text-secondary leading-relaxed">The Agent Fair-Trade Agreement is one answer to the question of how agents and the web should trade with each other. It is not the only possible answer. It is the answer we built first because we needed to. We invite better answers, harder questions, sharper criticisms, alternative drafts, and forks. The standard is too important to be owned by one publisher.</p>
<hr className="my-10 border-border" />
<h2 id="12-conclusion" className="text-2xl sm:text-3xl font-bold text-text-primary mt-12 mb-5 leading-tight scroll-mt-24">12. Conclusion</h2>
