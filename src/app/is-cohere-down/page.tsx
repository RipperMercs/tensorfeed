...
async function fetchCohereStatus(): Promise<StatusService | null> {
  try {
    ...
    if (res.ok) {
      ...
      if (data.ok && data.services?.length) {
        return (
          data.services.find(
            (s: StatusService) =>
              s.name === 'Cohere' || s.name.includes('Cohere')
          ) || null
        );
      }
    }
...
export const metadata: Metadata = {
  title: 'Cohere Status: Is Cohere Down? Live Cohere API Tracker',
  description:
...
function getStatusMessage(status: string): string {
  switch (status) {
    case 'operational':
      return 'Cohere is up and running normally. All systems are operational.';
    case 'degraded':
      return 'Cohere is experiencing degraded performance. Some features may be slower or intermittently unavailable.';
    case 'down':
      return 'Cohere is currently down. The Cohere team is likely aware and working on a fix.';
    default:
      return 'Unable to determine Cohere status at this time. Check back shortly.';
  }
}
...
function getStatusHeading(status: string): string {
  switch (status) {
    case 'operational':
      return 'Cohere is Operational';
    case 'degraded':
      return 'Cohere is Degraded';
    case 'down':
      return 'Cohere is Down';
    default:
      return 'Cohere Status Unknown';
  }
}
...
function getDynamicFaqAnswer(status: string): string {
  switch (status) {
    case 'operational':
      return 'No, Cohere is not down right now. All systems are operational and functioning normally.';
    case 'degraded':
      return 'Cohere is currently experiencing degraded performance. Some features may be slower than usual or intermittently unavailable.';
    case 'down':
      return 'Yes, Cohere appears to be down right now. The Cohere team is likely investigating the issue. Check back here for live updates.';
    default:
      return 'We are currently unable to determine Cohere status. Check back shortly for an update.';
  }
}

export default async function IsCohereDownPage() {
  const service = await fetchCohereStatus();
  ...
  const faqs = [
    ...
    },
    {
      ...
      answer:
        'Visit TensorFeed.ai/is-cohere-down for real-time Cohere status monitoring, or check the Cohere status page directly.',
    },
    {
      question: 'What do I do when Cohere is down?',
      answer:
        'You can try alternative AI services like Claude or GPT-4o while Cohere is experiencing issues. Check TensorFeed.ai/status for the status of all major AI services.',
    },
  ];
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <WebApplicationJsonLd
        name="Is Cohere Down? Live Cohere Status Monitor"
        description="Real-time Cohere status monitoring. Check if Cohere is down, experiencing degraded performance, or fully operational."
        url="https://tensorfeed.ai/is-cohere-down"
      />
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-bold text-text-primary">Is Cohere Down?</h1>
