...
function getStatusMessage(status: string): string {
  switch (status) {
    case 'operational':
      return 'Groq is up and running normally. GroqCloud and the API are operational.';
    case 'degraded':
      return 'Groq is experiencing degraded performance. Some models may be slower or intermittently unavailable.';
    case 'down':
      return 'Groq is currently down. Groq is likely aware and working on a fix.';
    default:
      return 'Unable to determine Groq status at this time. Check back shortly.';
  }
}
...
function getStatusHeading(status: string): string {
  switch (status) {
    case 'operational':
      return 'Groq is Operational';
    case 'degraded':
      return 'Groq is Degraded';
    case 'down':
      return 'Groq is Down';
    default:
      return 'Groq Status Unknown';
  }
}
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {service && service.components.length > 0 && (
        <section className="mb-8">
          <h2 className="text-xl font-semibold text-text-primary mb-4">Model Status</h2>
          <div className="bg-bg-secondary border border-border rounded-xl divide-y divide-border">
            {service.components.map((comp) => (
              <div key={comp.name} className="flex items-center justify-between px-5 py-3.5">
                <span className="text-sm text-text-secondary font-mono">{comp.name}</span>
