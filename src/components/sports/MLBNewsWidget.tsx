...
export default function MLBNewsWidget() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch('/api/sports/mlb/news?limit=15');
  ...
  if (articles.length === 0) {
    return (
      <div className="text-sm text-text-secondary py-4">
        No MLB articles yet. The hourly cron may not have fired since deploy.
      </div>
    );
  }
